...
using ByteBank.Portal.Infraestrutura.Filter;
...
namespace ByteBank.Portal.Infraestrutura
{
    public class ManipuladorRequisicaoController
    {
        ...
        public void Manipular(HttpListenerResponse resposta, string path)
        {
            ...
            var actionBinder = _actionBinder.ObterActionBindInfo(controller, path);

            var filterResult = _filterResolver.VerificarFiltros(actionBinder);

            if (filterResult.PodeContinuar)
            {
                var resultadoAction = (string)actionBinder.Invoke(controller);
